...
class GPNNReader(object):
  ...
  def _transferLabel2Onehot(self, labels, N):
  ...
  def _loadRedditFromNPZ(self, dataset_dir):
    adj = sparse.load_npz("./data/reddit/reddit_adj.npz")
    data = np.load("./data/reddit/reddit.npz")

    return adj, data['feats'], data['labels'], data['y_train'], data['y_val'], data['y_test'], data['train_index'], data['val_index'], data['test_index']

  def _load_reddit(self, prefix='./data/reddit/reddit', normalize=True):
    G_data = json.load(open(prefix + "-G.json"))
    G = json_graph.node_link_graph(G_data)
    # G_copy = json_graph.node_link_graph(G_data)
    ...
    # id_all = np.array([n for n in G.nodes()])
    train_ids = np.array([n for n in G.nodes() if not G.node[n]['val'] and not G.node[n]['test']])
    # train_feats = feats[train_ids]
    test_ids = np.array([n for n in G.nodes() if G.node[n]['test']])
    val_ids = np.array([n for n in G.nodes() if G.node[n]['val']])
    train_labels = np.array([class_map[i] for i in train_ids])
    test_labels = np.array([class_map[i] for i in test_ids])
    val_labels = np.array([class_map[i] for i in val_ids])
    # labels_all = np.array([class_map[i] for i in id_all])
    labels_all = np.zeros((numNode), dtype=int)
    for k, v in id_map.items():
      labels_all[v] = class_map[k]
    print(np.unique(labels_all, return_counts=True))
    train_index = np.array([id_map[i] for i in train_ids])
    val_index = np.array([id_map[i] for i in val_ids])
    test_index = np.array([id_map[i] for i in test_ids])
    print(len(train_index), len(val_index), len(test_index))
    print(train_index[-1], val_index[-1], test_index[-1])
    # print(len(class_map))
    print(len(G.edges()))

    newEdges0 = [id_map[edge[0]] for edge in G.edges()]
    newEdges1 = [id_map[edge[1]] for edge in G.edges()]

    print(len(newEdges0))

    adj = sparse.csr_matrix((np.ones((len(newEdges0),)), (newEdges0, newEdges1)), shape=(numNode, numNode))
    sparse.save_npz("./data/reddit/reddit_adj.npz", adj)

    np.savez('./data/reddit/reddit.npz', feats = feats, labels = labels_all, y_train=train_labels,
      y_val=val_labels, y_test = test_labels, train_index = train_index,
  ...
  def _get_data_dict(self):
    ...
    # load reddit dataset
      ...
      # self._transferG2ADJ()
      # self._transferRedditDataFormat(self._data_folder,"reddit.npz")
      # adj, features, y_train, y_val, y_test,train_index, val_index, test_index = self._loadRedditFromNPZ(self._data_folder)
      # self._load_reddit()
      adj, features, labels, y_train, y_val, y_test, train_index, val_index, test_index = self._loadRedditFromNPZ(self._data_folder)
      y_train = self._transferLabel2Onehot(y_train, 41)
      y_val = self._transferLabel2Onehot(y_val, 41)
      y_test = self._transferLabel2Onehot(y_test, 41)
      ...
      data_dict["train_label"] = y_train
      data_dict["test_feat"] = test_feat
      data_dict["test_label"] = y_test
      data_dict["all_feat"] = features
      print(np.unique(labels, return_counts=True))
      print(labels[0])
      data_dict["all_label"] = self._transferLabel2Onehot(labels, 41)
      print(data_dict["all_label"][0])
      data_dict["graph"] = nx.to_dict_of_lists(nx.from_scipy_sparse_matrix(adj))
      data_dict["test_id"] = test_index
  ...
  def _concat_data(self, data_dict):
    if self.dataset_name != "reddit":
      ...
      self._feat_all = self._preprocess_features(data_dict["all_feat"], norm_method=None)
      self._label_all = np.argmax(data_dict["all_label"], axis=1)
      print(len(data_dict["all_label"]))
      self._idx_all = np.arange(self.num_feat)
  ...
  def _get_split(self):
    if self.use_rand_split:
      ...
      split_val_idx = np.arange(self.num_nodes_train,
                                self.num_nodes_train + self.num_nodes_val)
      split_test_idx = np.arange(self.num_nodes_train + self.num_nodes_val,
                                 self.num_nodes_train + self.num_nodes_val + self.num_nodes_test)
  ...
  def _pack_data(self, data_dict):
    ...
    if self.dataset_name == "diel":
      ...
      data.node_gt_label = np.zeros(self._num_nodes, dtype=np.int32)
      print(len(feat_idx), self._label_all.shape, self._num_nodes, len(self._idx_all))
